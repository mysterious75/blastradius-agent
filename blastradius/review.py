...
REVIEW_PROMPT = (
    ...
    "- lifetime issues where the referenced object is heap-allocated and its ownership\n"
    "  is MOVED (e.g. kj::Own) into a longer-lived container before the local builder\n"
    "  dies — moving a kj::Own transfers the pointee; it does not free it\n"
    "- strict-aliasing/lifetime UB from placement-new of a v8 handle into a size_t slot\n"
    "  (the standard FFI handle-passing idiom, size/alignment static_asserted) — technically\n"
    "  UB per the standard, but no memory-safety impact, no attacker control, no demonstrated\n"
    "  miscompilation; it is the intended FFI design\n"
    "- concurrency claims where the signal handler runs on the SAME thread as the mutation\n"
    "  and synchronously to completion — a same-thread handler cannot hold a pointer across\n"
    "  a delete; atomic acquire/release discipline closes the window\n"
...
VERIFY_PROMPT = (
    ...
    "  any temporary dies)\n"
    "- the claimed-freed object is heap-allocated and its ownership is MOVED (kj::Own) into\n"
    "  the registry/container before the builder dies — the pointee stays alive at the same\n"
    "  address; moving a kj::Own does not free it\n"
    "- the claim is strict-aliasing/lifetime UB from placement-new of a v8 handle into a size_t\n"
    "  slot (FFI handle-passing idiom, size/alignment static_asserted) — technically UB per the\n"
    "  standard but with no memory-safety impact, no attacker control, and no demonstrated\n"
    "  miscompilation; it is the intended FFI design\n"
    "- the concurrency claim is same-thread (the signal handler runs on the same thread as the\n"
    "  mutation, synchronously to completion) — a same-thread handler cannot hold a pointer\n"
    "  across a delete; atomic acquire/release discipline closes the window\n"
