...
def test_minified_vendor_and_dist_files_are_skipped(tmp_path, hunter):
    _write(tmp_path, "static/js/app.min.js", "document.write('<img src=x onerror=alert(1)>');\n")
    _write(tmp_path, "vendor/lib.js", "el.innerHTML = payload;\n")
    _write(tmp_path, "dist/bundle.js", "document.getElementById('x').innerHTML = data;\n")
...
def test_migrations_directory_is_skipped(tmp_path, hunter):
    ...
    _write(tmp_path, "app.py", 'query = "SELECT * FROM users WHERE name = \'" + name + "\'"\n')
    findings = hunter.scan_repo(str(tmp_path))
