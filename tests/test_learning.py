...
def test_record_and_read_outcomes(improver):
    ...
    improver.record_outcome(
        make_finding(vuln_type="xss"),
        was_fp=False,
        sandbox_result="CONFIRMED_EXPLOITABLE",
        patch_confidence=100.0,
    )
...
def test_analyze_fp_rates(improver):
    ...
    for _ in range(2):
        improver.record_outcome(
            make_finding(vuln_type="sqli"), was_fp=False, sandbox_result="CONFIRMED_EXPLOITABLE"
        )
...
def test_payload_weight_for_proven_payloads(improver):
    for _ in range(3):
        improver.record_outcome(
            make_finding(vuln_type="sqli", payload="SELECT * FROM"),
            was_fp=False,
            sandbox_result="CONFIRMED_EXPLOITABLE",
        )
    rules = improver.update_rules()
...
def test_scanner_skips_learned_patterns(tmp_path, monkeypatch):
    ...
    (tmp_path / "repo").mkdir()
    (tmp_path / "repo" / "app.config.js").write_text("el.innerHTML = payload;\n", encoding="utf-8")
    hunter = CVEHunter()
